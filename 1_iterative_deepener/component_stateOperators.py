...
def updateState(st,earlTime,earlProc_idx,earlTask_idx,parent,startInd, t_lengths, p_speeds, debug)->list:
    '''
    Updates state with the next available processor and next task in waitlist.
    If waitlist empty, just free the processors.
    '''
    ...
    for i in range(0,len(st[0])):
        ...
        if earlProc_idx != -1 and earlTask_idx != -1: # If we are done filling up the processors...
            done.append(task_run[earlTask_idx]) # Start freeing jobs...
    ...
    if len(st[0])==0: # If the waitlist is empty, then the update become just popping off finished jobs...
        ...
        endTimes = [*st[10]]
        # 5. Done task update.
        if earlProc_idx != -1 and earlTask_idx != -1: # Free the jobs...
